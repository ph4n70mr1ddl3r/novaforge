...
/**
 * Binds {@link TenantContext} from the verified token's claims (PHASE-0 §6.2/T7):
 * tenant from the {@code tenant_id} claim, actor from {@code sub}. The token here is
 * the <em>calling user's</em> (relayed by the Data Runtime's hook call) — that is what
 * makes script execution caller-context (ARCHITECTURE.md §5 item 4). Requests without
 * a tenant claim leave the context unbound; downstream {@code require()} calls then
 * fail closed.
 */
@Component
@Order(0)
