 ...
 * never observed, so the {@code <topic>.DLT.novaforge-audit} publish was verified only
 * by mirror-construction. The budget is property-tunable now, and this context shrinks
 * it to 10 ms/10 ms/three attempts so the exhaustion — and the recoverer's publish of
 * the ORIGINAL payload — is watchable end to end against the real broker.
 */
...
class AuditDeadLetterTests extends PostgresTestBase {
    ...
    /** The production factory every audit listener resolves — the recoverer under test
    ...
    void exhaustedBudgetDeadLettersTheOriginalRecord() throws Exception {
        ...
        container.getContainerProperties().setGroupId("novaforge-audit-dlt-" + UUID.randomUUID());
        ...
        try {
            ...
            String deadLetterTopic = topic + ".DLT.novaforge-audit";
