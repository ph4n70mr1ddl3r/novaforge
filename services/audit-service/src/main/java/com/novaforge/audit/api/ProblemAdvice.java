...
/**
 * RFC 7807 problem+json with the common-core codes (PHASE-0 §5.2) — the same shape
 * every other service renders. Found by pinning the read API's limit bound (the one
 * {@code PlatformException} thrower on this surface): with no advice at all the
 * bound rejected as a raw 500 ServletException instead of the platform's 400
 * problem — every audit-side contract error would have rendered the same way.
 */
