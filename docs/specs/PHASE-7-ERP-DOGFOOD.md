...
traceability rises from requirements to the workflows themselves: the dogfood is
bound to **all** of erpplans' operational workflows, and every one of the 5,449
classification rows (5,426 canonical `## W` workflows + 23 parent/summary
sub-workflow rows) is in exactly one of three states in the committed workflow
register (`apps/buildright/workflow-coverage/`, generated by
`scripts/generate-workflow-coverage.py` from the upstream corpus + the
hand-maintained `workflow-map.json` + the requirement states of §12.3): (a)
**pinned** — the workflow's core cycle executes green in the e2e suite corpus, and
...
- **Wave 1 (shipped):** the Tier-1 procure-to-pay cycle (VS-15, R3/PUR) — PO with
  ...
  settlement by posted-payment roll-up, open-bills/spend reporting — the cycle's
  workflows (W2, W2A, W3, W7) pinned in the workflow register (§12.3a)
