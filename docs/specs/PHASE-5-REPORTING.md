...
| Addition | Detail |
|---|---|
| `novaforge-reporting-service` | Port 8089; gateway routes `/api/v1/reports/**` and `/api/v1/dashboards/**`. |
...
{ "id": "rep_ar_aging",
  ...
  "groupBy": [ { "field": "customerName" }, { "field": "agingBucket",
                 "buckets": [ { "label": "0-30", "expression": "daysOverdue <= 30" },
                              { "label": "31-60", "expression": "daysOverdue > 30 && daysOverdue <= 60" },
                              { "label": "60+", "expression": "daysOverdue > 60" } ] } ],
...
- **Authorization — pinned:** report runs execute as the *requesting actor* through
  ...
  reports exactly as to lists. No system-principal reporting; a manager's dashboard
  shows what that manager may see.
...
- `DashboardDefinition` = a grid of widgets `{ widget: kpi | chart | table,
  ...
  from three new versioned catalog components — `ChartWidget` (ECharts props
  schema), `KpiTile`, `DashboardGrid` — following the ADR-009 catalog contract
  (props JSON Schema, lazy, versioned). This is the composition PHASE-2 §13/Q4
  deferred here.
...
| T5 | Catalog components | `ChartWidget`, `KpiTile`, `DashboardGrid` + props schemas + stories (§5) | Catalog gallery green incl. axe |
