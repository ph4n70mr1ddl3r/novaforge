...
## Forty-Third Pass — 2026-09-08 (two completeness defects, each found by its own gate tripping: the coverage tracker's `\d{3}` id anchor never saw the catalog's four letter-suffixed requirements — the portfolio's definition-of-done undercounted the obligation it tracks, 724 vs the upstream canon of 728, with the gate pinning the defect twice — and the e2e stack's teardown rode shutdown hooks the surefire fork skips, leaking service jars that bricked the next run's preflight)

### C-43P1 — `generate-coverage.py`'s row regex anchored `\d{3}` immediately before the column break, so the erpplans catalog's four letter-suffixed requirements (POS-014a, NFR-022a, PUR-025a, PUR-025b) never parsed — the committed matrix totaled 724 against an upstream canon of 728 (429 Must / 293 Should / 6 Nice), every hand-quote of "724 requirements" in README/PLAN/PHASE-7 §12 derived from that parse, and `BuildrightAppArtifactTests` pinned the 724 twice

The erpplans catalog has carried the four letter-suffixed rows since its first commit (2026-06-08) — real 3-column rows with priorities and titles, and the upstream validator's own canon (Check 41: "728 requirements across 38 prefixes", split 429/293/6) counts them. NovaForge's generator — the tool PHASE-7 §12.3 makes the definition of done ("every requirement in erpplans' `erp-requirements.md` is in exactly one of two states") — parsed with `([A-Z]{2,4}-\d{3}) \|`, which demands the column break right after the third digit: `POS-014a |` has an `a` there, so all four rows silently fell out of `parse_requirements`, out of `coverage.json`'s requirements list, out of every total. The committed artifacts read 724 (1 covered / 188 partial / 535 uncovered; must-have 426 = 1/128/297) — four short, three of them Must Have. The failure is the repo's own anchor-satisfying-itself class, in the gate form: `coverageMatrixIsHonest` asserts "the matrix totals the erpplans catalog (724 rows)" and asserts the covered+partial+uncovered sum equals **the same 724 the defective parse produced** — the invariant's wording ("totals the full catalog") was satisfied off the tracker's own blind spot, so the gate could never catch the tracker. And the miscount escaped upward: README (×3), PLAN.md (×3), and PHASE-7 §12's pin all quote "724 requirements" as the size of the standing obligation.

**Fixed (V-C43P1):**

- **The regex** (`apps/buildright/scripts/generate-coverage.py`): the id group is now `[A-Z]{2,4}-\d{3}[a-z]?` — letter-suffixed rows parse with their priorities. The prefix derivation (`rsplit("-", 1)`) already handled the suffix; nothing else in the generator needed the shape.
- **The override wave**: an explicit `requirements` entry can now carry its own `wave` (previously the override branch always inherited the prefix's wave — an uncovered PUR row would have rendered "wave 1", the wave that shipped).
- **Two honest uncovered overrides** (`apps/buildright/coverage-map.json`): PUR-025a (commodity price-index tracking & forward-buy triggers) and PUR-025b (supplier innovation collaboration) would otherwise have inherited the PUR prefix default — *partial, wave 1, apps/buildright* — which overclaims: the wave-1 P2P app ships the PO→receipt→bill chain only, and the prefix's own note says replenishment-adjacent scope is wave 2+. Both are now explicit `uncovered`, wave 2, with the overclaim reason as evidence. POS-014a and NFR-022a ride their prefix defaults honestly (POS → uncovered/edge, integration-metadata-only per the sourcing doctrine; NFR → uncovered/platform, platform contracts not app metadata).
- **Regenerated** (`requirements-coverage/`): 728 total (covered 1, partial 188, uncovered 539); must-have 429 (1/128/300) — both now exactly the upstream canon's own split; POS 117→118, NFR 41→42, PUR 42→44 with 2 uncovered.

**Pinned (V-C43P2) — the gate learns the canon and the docs re-point:**

- `BuildrightAppArtifactTests` pins **728** (twice: the total and the status sum) plus the must-have tier total **429**, with a comment stating the contract the pins enforce: a failure means the upstream catalog grew — re-run `generate-coverage.py`, extend the coverage map consciously, re-point the pin. The defective invariant ("gate equals tracker") became the real one ("gate equals catalog").
- The doc re-points: README ×3, PLAN.md ×3, PHASE-7 §12 ×2 (the pin's blockquote gains a dated amendment noting the recount and its cause, per the ledger's records-stay-honest convention; §12.3's "724-row catalog" → "728-row catalog"). No other surface quoted the counts; `apps/buildright/README.md` never did.

**Bite-proven**: with `coverage.json`'s total injected back to 724, the gate fails at exactly the new pin — `[the matrix totals the erpplans catalog (728 rows)]`, 1 failed of 12; regenerated, 12/12. The pass also caught its own first draft: the two new overrides initially used a `note` key where the map's override convention (and the generator's reader) is `evidence` — the regenerated rows rendered empty notes, caught by reading the generator's output before landing, renamed, regenerated.

### C-43P2 — the e2e stack's teardown rode the surefire fork's shutdown hooks, which the fork skips on halt: two green verify runs leaked service jars (integration-service — the one service that survives infrastructure loss, its Kafka clients retrying forever after Ryuk kills Postgres/Kafka — plus a reporting-service), and the next run's port preflight bricked all five cycle tests on the held 8090

The pass's own full-reactor run tripped over it: run two's five e2e tests all died in `preflightPorts` on `port 8090 is already in use` — the integration-service jar of the green run before it still listening. The evidence trail: the leaked service's log shows zero shutdown activity at its own run's exit (no SIGTERM was ever delivered — `stopServices`' destroy ladder never ran), and the same single-cycle run that leaked before leaves nothing behind once teardown rides the launcher session's close.

**Fixed (V-C43P3):**

- **`StackTeardown`** (`e2e-tests`, a `LauncherSessionListener` registered via ServiceLoader): `launcherSessionClosed` calls the stack's new static, idempotent `shutdownSpawnedServices()` — destroy → 15 s grace → SIGKILL per process, executed while the test JVM still lives and owns its children, exactly the window the JVM hooks can't promise. The existing `Runtime` hook stays as belt-and-braces; `stopServices` delegates to the same ladder.
- **The preflight names the culprit**: a best-effort `ss -tlnp` + `/proc/<pid>/cmdline` hint appended to the port-occupied failure — `The listener is pid N (java -jar …/integration-service.jar)` — turning the diagnosis into a one-line kill; every probe failure is swallowed so the bind error stands alone. Born from this pass's own diagnostic cycle; bite-proven live below.
- **`e2e-tests/README.md`** states the new guarantee: the stack is torn down at the launcher session's close, no packaged jar outlives the run, JVM hooks are belt-and-braces only.

**Bite-proven**: with a squatter on 8090, the preflight failure names the holding pid and its command line; without it, the message named only the port. After the fix, a single-cycle module run that previously leaked integration-service + reporting-service (both killed by hand from run three's leftovers) leaves **zero** `SNAPSHOT.jar` processes immediately after BUILD SUCCESS.

### Verification (this pass)

- `BuildrightAppArtifactTests` 12/12 green on the fixed artifacts; bite-proof above.
- The full-reactor story, honestly: run one (`./mvnw verify`, pre-fix tree) BUILD SUCCESS in 13:03 — it leaked its integration-service jar silently, which run two's e2e phase then tripped over (the five preflight errors that exposed C-43P2); run three resumed at `-rf :novaforge-e2e-tests`, all five cycle tests green end to end — and leaked again, proving the class. On the fixed tree: the single-cycle module run green with zero orphaned jars, and the full e2e module (`verify`, all five cycles) green with zero orphaned jars after BUILD SUCCESS — the fix's own record. No product Java moved outside the e2e module's test sources; every other module's green run (runs one–three) remains valid for unchanged code.
- `pnpm -r check` + `pnpm -r test` green (302: shared 184, builder 74, runtime 44 — backend/frontend untouched by this pass; the runs keep the baseline claim honest). `deploy/helm/check-charts.sh` CLEAN (no deploy surface touched).
- Regeneration is stable: a second `generate-coverage.py` run reproduces 728/429 exactly (only the `generatedAt`/rev stamps move — the tool's designed point-in-time signature).
- Whole-repo sweep: no "724" remains outside this pass's own explanatory comments, the dated amendment, and CODE_REVIEW.md's historical record.

### Recorded open after this pass

Empty — with one honest note: the coverage-map's POS/NFR prefix notes render truer for the two new rows than per-row evidence would (POS-014a's senior-citizen/PWD discount compliance is a POS-edge integration seam; NFR-022a's BIR CAS registration rides the platform's retention track), but neither row has per-row evidence text in `matrix.md`'s claimed table — the matrix lists only covered/partial rows, and these are uncovered by design; their per-row notes live in `coverage.json` for the machine consumers.

## Forty-Fourth Pass — 2026-09-08 (the register gate read the artifact but never the map the pins are authored on: workflow-map.json and coverage-map.json could drift from their committed, canon-pinned artifacts with CI green — the pin-realism contract unenforced on the very surface claims are written — plus the "wave-attributed" claim that overclaims for 4,529 of the 5,214 uncovered rows, and four hygiene defects in the two generators)

### C-44P1 — both dogfood gates walk only the *generated* artifacts (`workflow-coverage/coverage.json`, `requirements-coverage/coverage.json`); the hand-maintained maps (`workflow-map.json`, `coverage-map.json`) — the surfaces claims are actually authored on, per the generators' own "edit the map and re-run" docstrings — were invisible to CI. A map edit that never met its generator shipped green with a stale register behind it, and `workflow-map.json`'s own comment advertised the exact enforcement the gate did not perform ("the register gate enforces both existence and membership in the live e2e corpus" — of the generated copy only)

The HEAD landing's pin-realism contract ("a pin is only as real as the executable that runs it") was enforced against `coverage.json` — a derived artifact whose pins are verbatim copies of the map's. The bite-proof in the landing's own message had to inject the bad pin into `coverage.json` to demonstrate it, because the gate has no leg that reads the map: a contributor adds a pin naming a nonexistent or corpus-external suite to `workflow-map.json`, forgets the regeneration, and the commit lands green — the register stays stale, the map claims a pin nothing executes, and neither leg of the advertised contract ever fires. The same shape on the requirement side: `coverage-map.json`'s statuses, evidence, waves, and prefix defaults feed `requirements-coverage/coverage.json` through the generator's merge, and nothing tied the two together — edit an evidence string (or typo a prefix key into `PURR`, applying to nothing, silently) without re-running `generate-coverage.py` and CI stays green. This is the forty-third pass's "gate equals tracker" class one level up: the gate pinned the artifact against the upstream canon (5,426 / 5,449 / 728 / 429 all hold) but left the artifact's *declared source* ungated, so the two committed files could disagree forever.

**Fixed (V-C44P1, two gate legs that make the maps first-class gate surfaces):**

- `BuildrightWorkflowRegisterTests.workflowMapStaysInSyncWithTheRegister`: every `workflow-map.json` entry must exist in the register and carry the register's exact status, sorted suites, app, note, and wave (the generator's merge, mirrored — a deterministic comparison, so a map edit *must* ship with its regeneration in the same commit); the pin-realism contract re-fires on the authored surface (suites must exist and ride the live corpus), and every named app — any status — must be known and on disk.
- `BuildrightAppArtifactTests.coverageMapStaysInSyncWithTheMatrix`: every `coverage-map.json` prefix key must be a real catalog prefix and vice versa (the generator's unknown-prefix exit-2 condition, now also a gate leg — the generator warns and still writes files, so exit codes alone never gated CI), every override id must name a catalog row, and every committed requirement row must equal the map's own merge (status/app/wave/note, the generator's logic mirrored exactly, including the override-wave-inherits-prefix-default and the missing-prefix fallback).

**Bite-proven three ways** (each restored after its proof): a `W9` note edit in the map without regeneration fails `[W9's map note diverges from the register — re-run scripts/generate-workflow-coverage.py]`; a `PURR` prefix key fails `[every coverage-map prefix is a real catalog prefix]`; a `PUR-001` evidence edit without regeneration fails `[PUR-001's committed note diverges from coverage-map.json]`. The committed tree is in sync (verified by the legs' green runs), so the gates land enforcement, not findings.

### C-44P2 — the uncovered state's "wave-attributed" claim overclaims for 87% of the uncovered register: the spec's §12.3a, both READMEs, PLAN's traceability bullet, the generator's docstring and matrix row, the map comment, and the ledger entry all promised wave attribution, while 4,529 of the 5,214 uncovered rows ride `null` waves — the upstream `requirement-workflow-matrix.md` covers the core value streams incrementally, and a workflow with no matrix link has nothing to derive a wave from

The register's own gate comment was the only honest surface ("the wave is attribution only (null where the upstream matrix does not yet link the workflow to any requirement)") — the docs stated the invariant the artifact contradicts, the exact prose-vs-artifact drift class the forty-second pass closed for the e2e ledger. **Fixed (V-C44P2)** by re-pointing every claim to the honest form — "wave-attributed where the upstream matrix links them (honestly null where it does not yet)": PHASE-7 §12.3a, README, PLAN.md, `apps/buildright/README.md`, `workflow-map.json`'s comment, the generator's docstring and matrix row (regenerated — the committed `matrix.md` now reads "Uncovered (visible; wave-attributed where the matrix links them)"), and a dated amendment on IMPLEMENTATION.md's ledger entry per the records-stay-honest convention. No counts moved (7 pinned / 228 partial / 5,214 uncovered, reconciled by the gate).

### C-44P3 — four hygiene defects in the two generators the register landing introduced or inherited: `parse_tiers` re-declared the module's own `TIER_HEADING` regexes as a byte-identical local (the constant dead since commit); `derive_wave` carried two dead parameters (`links`, `req_state`) at all three call sites; `generate-coverage.py`'s matrix writer computed `evidence` through a vacuous conditional (`... and not any(r["id"] == req["id"] for r in [])` — an always-false `any` over an empty list, both branches returning `req["note"]`); and `matrix.md`'s value-stream heading hardcoded "(188)" where the register itself counts (`len(vs_stats)` — 188 today, ungated)

**Fixed (V-C44P3)**: the local regexes deleted in favor of the module constant; the dead parameters dropped; the vacuous conditional collapsed to `evidence = req["note"]`; the heading derives from the computed count. Regeneration is byte-stable apart from the designed stamps (`generatedAt`/rev — which this pass's run also moved honestly forward to the HEAD rev, `5266622`, that the committed artifacts had not yet recorded) and the relabeled uncovered row.

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS on the podman socket across the 24-module reactor: **730 Java tests** green across the twenty test-bearing module summaries — 727 (the forty-second pass's recount) + 1 (the register gate itself, landed in HEAD after that recount) + 2 (this pass's two map-sync legs) — with all five e2e cycle tests green end to end (ErpSuiteCorpus 1, O2C 1, P2P 1, R2R 2 = 5) and zero orphaned service jars after BUILD SUCCESS (the forty-third pass's teardown holding). The two gate legs' bites above rode the same module.
- `pnpm -r check` + `pnpm -r test` green (302: shared 184, runtime 44, builder 74 — no frontend surface touched; the runs keep the README's count claim honest alongside the Java recount).
- `deploy/helm/check-charts.sh` CLEAN (12 charts lint+render, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets — no deploy surface touched).
- Whole-repo sweep: all relative markdown links resolve; zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; no "724" outside the historical/amendment record; "wave-attributed" claims now honest on every surface (grep-verified).

### Recorded open after this pass

Empty — with one honest note: the two sync legs mirror the generators' merge logic in Java (deterministic, ~40 lines each, side-commented as the mirror). If a generator's merge semantics ever change, its Java twin must change in the same commit — the gate failures read exactly like that instruction ("diverges from ... — re-run scripts/generate-...py"), and the coupling is the price of gating a hand-authored surface a generator stands between CI and.

## Forty-Fifth Pass — 2026-09-08 (the map-sync gate walked the map but never the register: deleting an authored pin shipped green with the stale register row still claiming it, and a pin omitting its `wave` key was never wave-compared at all — plus the ledger's partial count miscounting the register it describes, 227 vs 228, and a corrupted token in §12.7)

### C-45P1 — `workflowMapStaysInSyncWithTheRegister` iterated the map's entries only: the reverse drift — DELETING an authored pin — left the register's row standing with no authored source behind it, CI green. And the same leg's wave comparison ran only `if (pin.containsKey("wave"))`, so the generator's derived-wave fallback — a pure function of committed inputs — was the one merge branch with no mirror

The forty-fourth pass made the maps first-class gate surfaces, and its bite-proofs all edited or added map entries — every proof pushed in the map→register direction. The other direction never had a leg: `workflowMapStaysInSyncWithTheRegister` walks `workflow-map.json`'s entries, so removing an entry *removes the comparison*. A contributor deletes the W2 pin (or W9, the hand-noted partial) without regenerating, and the committed register still says `pinned` (or carries W9's hand note) with the suite pins intact — `workflowRegisterIsHonest` checks pinned rows' suites against the corpus, never against the map, so every gate stays green while the register claims a pin nothing authors anymore. The requirements twin never had this hole, for a structural reason worth stating: `coverageMapStaysInSyncWithTheMatrix` walks the *artifact* side (all 728 committed rows against the map's merge), so it sees drift in either direction. And within the same leg, the wave comparison was gated on the map entry *carrying* a `wave` key — the generator's `derive_wave` fallback (the branch its own docstring documents: `entry.get("wave", derive_wave(...))`) had no Java mirror, even though its inputs — the register row's `requirements` links and `coverage-map.json`'s prefix waves — are committed files a gate can read. A coverage-map wave edit that regenerated `requirements-coverage/` but not `workflow-coverage/` stayed green on every pin that omitted the key.

**Fixed (V-C45P1, two legs and a mirror in `BuildrightWorkflowRegisterTests`):**

- **The register→map direction**: every register row whose state only a map entry can produce must still have one, compared field-for-field through the same assertion the map-side walk uses. Which rows are map-reachable? `pinned` (pins are the only source), `partial` with a hand-authored note (a derived partial's note is exactly the generator's template — `exercises REQ (status via app); no dedicated suite pin` — mirrored as a regex predicate, side-commented with the coupling convention), and `uncovered` with an empty note (a derived uncovered note is never empty). Today that predicate selects exactly the 8 rows the map authors (the 7 pins + W9); the walk lands enforcement, not findings.
- **The derived-wave mirror**: `assertPinMatchesRegister` now compares the wave through both merge branches — an explicit map wave, or `derive_wave` mirrored exactly (minimum integer over the linked requirements' prefix waves, coverage-map prefix *defaults* — never a per-requirement override wave; else the first non-integer in primary-then-supporting order; else null), with the failure message naming the branch that fired (`map` vs `map-derived`).
- The field comparisons were extracted into the shared helper, so both directions assert identically — the same equality the generator's merge produces, now with nowhere to stand.

**Bite-proven both ways** (each restored after its proof): deleting W2's map entry fails `[the register's W2 row is pinned — a state only the map can author — but workflow-map.json has no entry for it (the pin was deleted without regenerating?) — re-run scripts/generate-workflow-coverage.py]`; dropping W2's `wave` key and staling the register's derived wave fails `[W2's map-derived wave diverges from the register — re-run scripts/generate-workflow-coverage.py]` — the leg that was silently skipped before. Restored, 2/2 green on the landed tree.

### C-45P2 — IMPLEMENTATION.md's workflow-register ledger entry says "**partial** … 227 rows": the committed register, the gate's pins, and the landing's own commit message all say 228 (7 pinned / 228 partial / 5,214 uncovered = 5,449) — a miscount at authoring time that the entry's own forty-fourth-pass amendment corrected everything around except the number itself

The entry's amendment even quotes the honest counts for the wave-attribution re-point ("4,529 of the 5,214 uncovered rows ride `null` waves") without noticing the partial count one clause earlier was wrong: 7 + 227 + 5,214 = 5,448 — the entry's own arithmetic doesn't reconcile to the 5,449 it pins in the same sentence. **Fixed (V-C45P2)**: re-pointed to 228 with a dated amendment (the records-stay-honest convention) naming the miscount and its source — the artifact, the gate, and the landing message. Repo-wide sweep: no other surface quotes 227.

### C-45P3 — PHASE-7 §12.7's closing sentence carries a corrupted token: "changed ones may invalidate **c claims**" — a stray fragment from the spec's own (a)/(b)/(c) state labels that broke a sentence every erpplans-revision walkthrough reads

**Fixed (V-C45P3)**: "may invalidate claims" — in place; a corrupted word is corrected, not amended (the C-42P2 precedent).

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS across the 24-module reactor on the podman socket: **730 Java tests** green across the twenty test-bearing module summaries — exactly the README's count, unchanged by this pass (the gate grew legs and helpers, not `@Test` methods) — with all five e2e cycle tests green end to end and zero orphaned `SNAPSHOT.jar` processes after BUILD SUCCESS (the forty-third pass's teardown holding).
- The two dogfood gates re-run green on the landed tree (`BuildrightAppArtifactTests` 13/13, `BuildrightWorkflowRegisterTests` 2/2, including both new legs); bites above rode the same class.
- `pnpm -r check` + `pnpm -r test` green: 302 frontend tests (shared 184, runtime 44, builder 74 — no frontend surface touched; the runs keep the README's count claim honest).
- `deploy/helm/check-charts.sh` CLEAN (12 charts lint+render, infra embedded files byte-identical, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` clean (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` — which this pass's own working tree tripped on the way in (stale compiled SPA assets in the gateway's `target/classes` from an old incremental build; the gate naming the exact files is it working as designed, and `mvn -pl services/gateway clean` resolves it) — CLEAN after the clean.
- Regeneration byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev` — the artifacts now record the HEAD rev `cd8239f`, which the committed copies had not yet recorded); the two bite-restored files re-generated and the gates re-verified green on the final tree.
- Whole-repo sweeps: all relative markdown links resolve; zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; no other quote of the 227 partial count and no other instance of the corrupted token (remaining grep hits are workflow IDs — W227 — and unrelated "spec claims" prose); skaffold's eleven artifacts/overrides and the CI workflows' SHA pins re-checked against the chart set.

### Recorded open after this pass

Empty — with two honest notes: the register→map predicate couples to the generator's derived-note template (the regex is side-commented with the change-in-the-same-commit convention, as the merge mirrors already were); and the mirror's exactness boundary stays where the forty-fourth pass drew it — derived partial/uncovered rows *without* map entries trust the committed register (gating them means re-running the generator's whole 5,449-row join in Java, the trade the forty-fourth pass already priced), while every row whose state a human authored is now matched from both sides.

## Forty-Sixth Pass — 2026-09-08 (the wave vocabulary's documented source never documented half its values: the buildright README's wave table stopped at wave 3 while the map, the register, and 315 workflow rows ride waves 4 and "platform" — plus the erp README's artifact inventory missing two of its ten suites, the loading loop loading eight, a dead teardown method the pass-43 refactor orphaned, and a gate comment flattening the corpus's own five-plus-three split)

### C-46P1 — `apps/buildright/README.md`'s Waves table enumerated waves 1/2/3/edge and stopped there, while `coverage-map.json` assigns `wave: 4` to 22 prefixes (253 requirement rows; 315 workflow rows wave-attributed through them) and `wave: "platform"` to NFR (42 requirement rows; 51 workflow rows) — and the map's own comment says "Waves follow apps/buildright/README.md", naming a vocabulary source that documented neither value

PHASE-7 §12.4 defines wave 4 ("the long tail — governance, loss prevention, ESG, services") and PLAN.md's Phase-9 bullet repeats it; the committed artifacts carry both values in every `byWave` tally the gates pin around. But the README the map points at as the wave vocabulary's source had no wave-4 row and no platform row — a reader reconciling the register's `byWave` (`{'4': 315, 'platform': 51, …}`) against the README's table found values with no definition, and the map's pointer advertised a contract its target didn't honor. The forty-second pass closed this exact class for the e2e ledger ("the README … had drifted four ways behind the corpus it describes"); this is the same drift one surface over. **Fixed (V-C46P1)**: the wave table gained the wave-4 row (mirroring §12.4's wording, pointing at `coverage-map.json`'s wave-4 prefix defaults as the assignment surface, with the 22 prefixes named — verified set-equal against the map by script, and marked "today" so upstream growth reads as a re-point, not a lie) and a platform row (NFR: platform contracts per PLAN.md §1, tracked by the platform phases and the perf harness, not app metadata — the map's own note, now visible where the comment claims it lives). The map's values and the README's rows now reconcile 1:1.

### C-46P2 — `apps/erp/README.md`'s artifact inventory described 8 of the 10 suite files in `apps/erp/suites/`: `orderToCash.json` and `recordToReport.json` — the O2C/R2R e2e-cycle suites, run green by `ErpOrderToCashE2ETest`/`ErpRecordToReportE2ETest`, members of the register gate's thirteen-suite `LIVE_E2E_CORPUS` — appeared in no table row, and the README's own loading loop uploaded only 8 of 10

The table is the README's inventory of the app directory's artifacts; the loop is its by-hand bring-up recipe. Both predated the e2e corpus landing (fde035e) and were never extended: a reader loading the app by hand produced an artifact missing two suites the e2e cycles and the pin-realism contract treat as first-class corpus members. **Fixed (V-C46P2)**: both suites got inventory rows (honest one-line descriptions derived from the suites' own case names — the O2C approval/auto-journal/payment-application/aging journey; the R2R P&L-and-ledger pin, soft close with the close-journal exemption, hard lock, reopen) and the loading loop now uploads all ten before publishing.

### C-46P3 — `NovaForgeStack.stopServices()` was dead code: a private, never-called method left behind by the forty-third pass's teardown refactor, delegating to the launcher-session teardown that replaced its role

The pass-43 landing moved teardown into `StackTeardown` (launcher-session close) + the belt-and-braces JVM hook; the old instance-method seam survived the move with no caller and no purpose — unread-execute dead weight in the file the pass was fixing, invisible to every gate (no unused-private-method check rides the build). **Fixed (V-C46P3)**: deleted; `shutdownSpawnedServices()` and its javadoc carry the whole story. Compilation is the proof nothing called it.

### C-46P4 — `BuildrightWorkflowRegisterTests`' corpus comment called all eight of `ErpSuiteCorpusE2ETest`'s suites "the eight acceptance suites", flattening the split every other surface draws: the e2e README, the main README, and the test class's own javadoc all say five acceptance suites *beside* three workflow-edge suites

A comment inside the pin-realism gate misdescribing the corpus the gate enforces is the prose-drift class the forty-second and forty-fourth passes kept paying down — small, but the gate is the contract's spine and its comment should use the corpus's own vocabulary. **Fixed (V-C46P4)**: the comment now reads "the eight-suite Phase-7 corpus (the five acceptance suites beside the three workflow-edge suites)".

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS on the podman socket across the 24-module reactor: **730 Java tests** green across the twenty test-bearing module summaries (unchanged — this pass touched a comment, a dead method, and markdown; no `@Test` count moved), with all five e2e cycle tests green end to end (ErpSuiteCorpus 1, O2C 1, P2P 1, R2R 2 = 5) and zero orphaned `SNAPSHOT.jar` processes after the runs (the forty-third pass's teardown holding). The two touched Java files were additionally re-verified against their edited sources: the two Buildright gates re-run green (`BuildrightAppArtifactTests` 13/13, `BuildrightWorkflowRegisterTests` 2/2) and the e2e module re-run green 5/5.
- `pnpm -r check` + `pnpm -r test` green: **302 frontend tests** (shared 184, runtime-ui 44, builder-ui 74 — exactly the README's count claim; no frontend surface touched).
- `deploy/helm/check-charts.sh` CLEAN (12 charts lint+render, infra embedded files byte-identical, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` clean (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` CLEAN.
- Regeneration byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev` — the artifacts now record the HEAD rev `6a40724` the committed copies had not yet recorded); counts reconcile (7/228/5,214 = 5,449; 1/188/539 = 728; must-have 1/128/300 = 429; byWave `{'1': 85, '2': 209, '3': 147, '4': 315, 'None': 4,529, 'edge': 113, 'platform': 51}`).
- Whole-repo sweeps: all relative markdown links resolve (including the two edited READMEs); zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; the wave-table fix verified set-equal against `coverage-map.json`'s wave-4 prefixes by script; the erp gap-log surfaces re-checked in sync (18 app entries = 16 G-rows + the two `—`-id deferral rows).

### Recorded open after this pass

Empty — with one honest note: the wave-4 row names the 22 prefixes explicitly (readers shouldn't have to derive the set from the map to see what "long tail" means), which makes it a hand-maintained list a map edit can stale — the "today" marker is the honest flag, the gate doesn't read prose, and the alternative (dropping the enumeration) trades a staling risk for a vaguer contract. The coverage-map comment remains the pointer, and the same change-in-the-same-commit convention that binds the maps to their generators binds this row to them.

## Forty-Seventh Pass — 2026-09-09 (four surfaces describing artifacts that moved under them: the register's 5,449-row contract still quoted as 5,426 where the re-point never landed, the erp README still selling G-2's retired script as its one budgeted escape hatch, the e2e stack's boot comment still counting the nine JVMs its own commit shrank to five, the pass-43 teardown story still credited to "Ryuk + shutdown hooks" in the two surfaces the refactor didn't revisit — plus the top-level README's repository layout missing two of the four apps it inventories)

### C-47P1 — the top-level `README.md`'s Why-NovaForge paragraph said "all 5,426 workflows register-pinned, requirement-derived, or uncovered", quoting the upstream canon where every sibling surface states the register's 5,449-row contract

The register-landing commit (5266622) wrote the sentence with 5,426 and re-pointed "both READMEs" in the same landing — but the top-level README's copy kept the canonical count, and the forty-fourth pass's wave-wording fix on that very line touched only the attribution clause, not the count. The contract the sentence describes — the register's state universe — is 5,449 classified rows (5,426 canonical `## W` workflows + 23 parent/summary sub-workflow rows) per PHASE-7 §12.3a, PLAN.md's traceability bullet, the buildright README's portfolio rule 3, the register's own totals row ("5,426 canonical + 23 sub-workflows = 5449"), and the gate's canon pins. Strictly, "all 5,426" is true (every canonical workflow is in exactly one state) — but it is the only surface that omits the 23 sub-workflow rows from the contract statement, and it is the sentence a newcomer reads first. **Fixed (V-C47P1)**: "all 5,449 classified workflow rows register-pinned, requirement-derived, or uncovered" — the same re-point PLAN.md and the buildright README already carry.

### C-47P2 — `apps/erp/README.md` still sold the app's escape hatch as live: "exactly one budgeted escape-hatch script … the budget is exceeded, 1 script of 4 hooks = 25%, under G-2's reviewed exception" (intro) and "The **one budgeted script** (`costMovement`, beforeSave) costs issues at the running average" (module map) — while the artifact has four hooks, zero scripts, and G-2 closed on 2026-09-03 with the costing re-authored as §3.7's declarative `bind` flow

The spec (PHASE-7 §3.7/§5), the gap log (G-2's disposition: "the script is gone (0 of 4 hooks — the ceiling holds)"), IMPLEMENTATION.md's ledger, and the artifact itself (`erp-app.json`: 4 hooks, all flows — verified by script) all carry the adoption; the README the artifact's inventory table lives in kept the pre-harvest story in two places, including a "the budget is exceeded" claim that is now the opposite of the truth. The forty-second pass re-authored the suites and the exit re-run rode the declarative flow; the README prose never followed. **Fixed (V-C47P2)**: the intro now states the live state (zero escape-hatch scripts; the costing hook entered as G-2's one budgeted script and left with the G-2 harvest — §3.7's `bind` flow, adopted 2026-09-03; 0 of 4 hooks, the ≤ 20% ceiling holds), and the Inventory module row describes the actual `costMovement` flow from the artifact (issue binds the `Item` roll-up and prices at `inventoryValue / qtyOnHand` — §3.5's post-adoption divisor; receipt stamps `value = qty × unitCost`), crediting the script as history, pinned unchanged by the `inventoryCosting` suite.

### C-47P3 — `NovaForgeStack.startServices()`'s boot-order comment said "so a 2-CPU runner never boots nine fat contexts into each other" — and the commit that wrote the comment (c4532d1) is the commit that shrank the boot list to five

The pre-shrink list booted eight services (plus Maven's JVM making nine — the commit message's "nine JVMs plus Maven plus four containers exceeded a 7 GB runner"); the fix cut `SCRIPT`/`AUDIT`/`NOTIFICATION` from the list, rewrote the comment around it, and carried the stale nine into the new text. A comment inside the boot loop miscounting the loops' own boots — invisible to every gate, wrong from the commit that introduced it. **Fixed (V-C47P3)**: "never boots its five fat contexts into each other" — the count of the `order` list the comment annotates.

### C-47P4 — the pass-43 teardown story was still untold in the two surfaces the refactor didn't revisit: `NovaForgeStack`'s class javadoc ("booted once per test JVM, reaped by Ryuk + shutdown hooks") and IMPLEMENTATION.md's e2e-stack section ("singleton; Ryuk + shutdown hooks reap it") — both pre-date the discovery that the surefire fork halts past its shutdown hooks, the discovery the forty-third pass exists to record

The truth the code now implements (and `StackTeardown`'s javadoc, the e2e README, and CODE_REVIEW's own C-43P2 all state): the launcher session's close tears the spawned services down while the JVM still owns its children — destroy → 15 s grace → SIGKILL; the JVM shutdown hook is belt-and-braces only; Ryuk reaps the *containers*, never the host service JVMs. The two stale surfaces credit the exact mechanism the pass-43 landing proved unreliable — in the file the forty-sixth pass edited (deleting the dead `stopServices` method without noticing the class javadoc three lines above it still told the pre-refactor story). **Fixed (V-C47P4)**: the `NovaForgeStack` singleton javadoc now names the teardown chain in order (launcher-session close via `StackTeardown`, belt-and-braces hook, Ryuk for the containers), and IMPLEMENTATION.md's e2e-stack paragraph carries the same story with the pass-43 lesson named.

### C-47P5 — the top-level `README.md`'s Repository Layout inventoried two of the four `apps/` directories: `apps/purchasing/` (the Phase 4 §1 exit journey as a versioned app, CI-gated by `PurchasingAppArtifactTests` since 68eb33e) and `apps/perf/` (the `PerfHook` fixture behind the 1M-row measurements of ARCHITECTURE.md §9 and `docs/loadtests/`) appeared in no row

The tree is the project entry point's own map of the repository; both omitted directories are CI-gated or measurement-bearing artifacts a new contributor would otherwise discover only by stumbling over them. The same inventory-drift class the forty-sixth pass closed for the erp README's suite table (C-46P2), one surface up. **Fixed (V-C47P5)**: both directories got layout rows — purchasing as the Phase 4 exit journey (machine + suite + its artifact test), perf as the §9-measured fixture — with wording sourced from the artifacts' own descriptions.

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS on the podman socket across the 24-module reactor on the final tree (docs, README, and the edited `NovaForgeStack.java` all in): **730 Java tests** green across the twenty test-bearing module summaries — exactly the README's count claim, unchanged (this pass touched comments and markdown; no `@Test` count moved) — with all five e2e cycle tests green end to end (ErpSuiteCorpus 1, O2C 1, P2P 1, R2R 2 = 5), the edited stack file compiled and exercised live. Counting note: local `target/` surefire dirs still hold stale reports for two test classes deleted on Sep 6 (`ProbeTest`, `FuzzDiffTest` — one carrying an old RED from its final pre-deletion run); the 730 counts only reports from this run — CI's clean checkout never sees them, and `mvn clean` is local hygiene, not a repo defect.
- The metadata-service module's run inside the verify covers the dogfood gates on the final tree (`BuildrightAppArtifactTests`, `BuildrightWorkflowRegisterTests`, `ErpAppArtifactTests`, `PurchasingAppArtifactTests`, `PerfAppArtifactTests` all green — 89 module tests total).
- `pnpm -r check` + `pnpm -r test` green: **302 frontend tests** (shared 184, runtime-ui 44, builder-ui 74 — exactly the README's count; no frontend surface touched; run on this tree pre-edit, and no edit since touches it).
- `deploy/helm/check-charts.sh` CLEAN (12 charts lint+render, infra embedded files byte-identical, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` clean (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` CLEAN (run pre-edit; no edit touches a surface it reads).
- Regeneration byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev` — the artifacts now record the HEAD rev `3948547` the committed copies had not yet recorded); counts reconcile (7/228/5,214 = 5,449; 1/188/539 = 728; must-have 1/128/300 = 429; byWave `{'1': 85, '2': 209, '3': 147, '4': 315, 'None': 4,529, 'edge': 113, 'platform': 51}`).
- Whole-repo sweeps: all relative markdown links resolve (including the three edited docs); zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; the artifact-side claims of the erp README's module map re-verified against `erp-app.json` by script (jobs, connector, webhook, reports, dashboard, workflow, machines — all present); the `derive_wave` Java mirror re-checked against the generator's three call sites (map fallback, derived partial's claimed-only set, derived uncovered's full links) — the mirror is only ever consulted on the map-fallback branch, where the two agree exactly.

### Recorded open after this pass

Empty — with one honest note, inherited and unchanged: the generator's derived-note template and the wave-vocabulary's "today" markers remain hand-maintained surfaces the gates cannot read (the forty-fifth and forty-sixth passes' notes stand). This pass adds no new gating — every finding was a surface describing an artifact that had moved, which is the drift class the change-in-the-same-commit convention (and these passes) keep paying down; the maps, registers, and gates themselves reconciled byte-clean.

## Forty-Eighth Pass — 2026-09-09 (the wave vocabulary's edge row never covered the fourth edge: the README enumerates POS/WMS/ecommerce while the map rides a TMS prefix through the same wave — plus the workflow generator's docstring quoting the state universe as 5,426 where the register it writes is 5,449 rows, the matrix's tier split labeled with a word the generator defines as a different set, and the architecture companion's repo layout predating the dogfood)

### C-48P1 — `apps/buildright/README.md`'s Waves-table edge row enumerated three of `coverage-map.json`'s four edge prefixes: the map assigns `wave: "edge"` to POS, WMS, ECOM **and LOG** (transport/TMS, R20 — 3 requirement rows, 3 of the 113 edge-wave workflow rows through them), and the row named the first three only

The forty-sixth pass fixed this table's wave-4 and platform rows — values the map carried that the README (the surface the map's own comment names: "Waves follow apps/buildright/README.md") did not document — and set-verified exactly those rows. The edge row escaped the same audit: it reads as an enumeration ("POS (R5), WMS (R4), ecommerce (R6)"), and its set is one short of the map's. The missing value is not exotic: PHASE-7 §12.5, PLAN.md's Phase-9 bullet, and the README's own rule 6 all name TMS among the bought edges, the map's `LOG` entry says "Transport management is a bought edge (TMS)", and the register's byWave tally the gates pin around carries its 3 workflow rows inside `edge: 113`. A reader reconciling the documented vocabulary against the assignment surface found a prefix riding a wave the vocabulary never offered it. **Fixed (V-C48P1)**: the edge row now reads "POS (R5), WMS (R4), ecommerce (R6), transport/TMS (R20's `LOG` prefix)" — and both enumerable rows are script-verified set-equal against the map (wave 4's 22 prefixes re-confirmed; the edge row's four names now cover exactly the map's `ECOM/LOG/POS/WMS`).

### C-48P2 — `generate-workflow-coverage.py`'s docstring still quoted the state universe as 5,426: "States (the workflow-level twin of §12.3's requirement states — every one of the 5,426 workflows is in exactly one)" — while the register the generator writes puts all **5,449** rows in exactly one state, and the same docstring's previous block says so

The register-landing commit (5266622) wrote the docstring; the canon split (5,426 canonical + 23 sub-workflows) lives four lines above the drifted sentence, and the gate pins `total = 5,449` with `pinned + partial + uncovered = total` — the state universe is the register, not the canonical subset. The forty-seventh pass fixed this exact class on the top-level README ("all 5,426" → "all 5,449 classified workflow rows"); the generator's own docstring kept the older count in the one sentence that defines the states. **Fixed (V-C48P2)**: "every one of the 5,449 register rows is in exactly one".

### C-48P3 — the same generator's matrix line "Tier split (universe):" labeled with a word its own vocabulary defines as a different set: the split counts all 5,449 register rows, but `universe` in this script is the 5,426 canonical workflow set (`parse_universe`, the `WF_HEADER` comment)

The line's numbers (1,396/3,295/758, summing 5,449) are the register's canon tier split — the register block's own note says "the canon tier split 1,396/3,295/758 counts all rows" — so the honest label is the one the note uses. A reader taking "(universe)" at the script's definition reconciled 5,449 against 5,426 and found the drift the label itself created. **Fixed (V-C48P3)**: the line is now "Tier split (register):", regenerated into the committed matrix — the only content change the regeneration produced apart from the designed stamps.

### C-48P4 — ARCHITECTURE.md §7's repo layout predated the dogfood: it inventoried platform/services/frontend/deploy/docs and omitted the `apps/` tree (all four directories — `erp`, `buildright`, `purchasing`, `perf`, each CI-gated or measurement-bearing) and `e2e-tests/` (the reactor's last module) entirely

The forty-seventh pass fixed this class on the top-level README's Repository Layout (C-47P5 — two of four apps missing there); the architecture companion's "Suggested Repo Layout" kept the pre-dogfood shape — no `apps/` at all, so the entire coverage-tracked corpus the project's standing obligation rides appeared in no layout row of the doc a new contributor reads second. **Fixed (V-C48P4)**: the tree gained an `apps/` block (four rows, one line each, sourced from the top-level README's own descriptions: the Phase 7 acceptance dogfood + gap log; the Phase 9 portfolio dogfood, coverage-tracked against erpplans; the Phase 4 exit journey as a versioned app; the 1M-row measurement fixture behind §9) and an `e2e-tests/` row (the whole-platform ERP cycles, the reactor's last module).

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS on the podman socket across the 24-module reactor on the final tree (all four fixes in, the regenerated artifacts committed): **730 Java tests** green, counted from this run's own surefire reports — exactly the README's count claim, unchanged (this pass touched markdown, a generator script, and generated artifacts; no `@Test` count moved) — with all five e2e cycle tests green end to end (ErpSuiteCorpus 1, O2C 1, P2P 1, R2R 2 = 5) and zero orphaned service JVMs after the runs (the forty-third pass's teardown holding).
- The two dogfood gates green on the final tree (`BuildrightAppArtifactTests` 13/13, `BuildrightWorkflowRegisterTests` 2/2 — including both map-sync directions over the untouched map).
- `pnpm -r check` + `pnpm -r test` green: **302 frontend tests** (shared 184, runtime-ui 44, builder-ui 74 — exactly the README's count; no frontend surface touched).
- `deploy/helm/check-charts.sh` CLEAN (12 charts lint+render, infra embedded files byte-identical, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` clean (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` CLEAN.
- Regeneration byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev` — the artifacts now record the HEAD rev `f29d6f8` the committed copies had not yet recorded) and the one designed content change (C-48P3's relabeled line); counts reconcile (7/228/5,214 = 5,449; 1/188/539 = 728; must-have 1/128/300 = 429; byWave `{'1': 85, '2': 209, '3': 147, '4': 315, 'None': 4,529, 'edge': 113, 'platform': 51}`); the edge and wave-4 rows set-verified against `coverage-map.json` by script.
- Whole-repo sweeps: all relative markdown links resolve (including the two edited docs); zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; no other "(universe)" label and no other state-universe quote of 5,426 (the remaining 5,426 hits are the upstream-canon quotes — the correct usage everywhere they stand).

### Recorded open after this pass

Empty — with one honest note, inherited and now stated: the Waves table's two scope-anchor rows (waves 2 and 3) name §12.4's erpplans anchors (R1/R11; MDM/CCR/VPP) while the map assigns four wave-2 and six wave-3 prefixes (INV rides wave 2 through its prefix default; COM/DOC/SCP join wave 3) — those rows describe scope, the map is the assignment surface, and only the enumerating rows (wave 4 "today", edge, platform) claim set-equality with it; the same change-in-the-same-commit convention that binds the maps to their generators binds those enumerations to the map.

---

## Forty-Ninth Pass — 2026-09-09 (the newest code's gate claimed the run-time checks it did not yet perform: the G-11 harvest's save-time awaitTasks gate admitted values its own run-time parse refuses — a JSON `2.5` or `true` stored green and only died mid-run — and the gate shipped without the DefinitionLifecycleTests rejection legs every other step-op gate carries — plus the Status counts lagging the feat commit's own test leg, 730 → 731)

### C-49P1 — `DefinitionService`'s save-time `awaitTasks` gate shape-checked its poll slots instead of parsing them: a `Number` with `intValue() > 0` passed (so `timeoutMs: 2.5`, `count: 2.0` — JSON decimals — stored green) and any non-string non-number passed (`count: true`), while the run-time's `positiveInt` does `Integer.parseInt(String.valueOf(value))` and refuses every one of them mid-run — the exact save/run split the gate's own comment disclaims ("numeric-positive-checked here, at save time, not at run time"), landed with the G-11 harvest in the same commit that grew the register to ten pins

The forty-eighth pass's recorded-open note was empty; the feat commit (`1fba240`) landed hours later carrying the harvest this pass reviews. The gate's shape check (`value instanceof Number number && number.intValue() <= 0`, `value instanceof String text && !isReference(text) && !text.matches("\\d+")`) answers a different question than the runtime's parse: Jackson lowers a JSON `2.5` to a `Double` whose `intValue()` is `2`, a `true` is neither branch, and both sail into the suite only to abort their case at run time with the very `VALIDATION_FAILED` the save gate exists to surface at authoring time (the same convention PHASE-4 §12's scanSla gate pinned: "authoring errors surface here, not as an aborted case at run time"). The cap rode the same hole: `timeoutMs: 120000` passes `intValue() > 0` and dies at run time on the 60000 ceiling. **Fixed (V-C49P1)**: the gate now mirrors the run-time checks exactly — absent slots and `${…}` references still pass (they resolve or default at run time), everything else must survive `Integer.parseInt(String.valueOf(value))` with `> 0`, `timeoutMs` additionally `<= 60000` — and the comment says what it now does.

### C-49P2 — the same save-time gate shipped with no rejection leg in `DefinitionLifecycleTests`, where every other step-op gate's rejection is pinned: resolveTask's action set, deleteRecord's version, scanSla's exactly-one-clock, queryRecord's v1 filter — the awaitTasks gate alone had no leg proving it fires

The test method's own comments state the convention ("authoring errors surface at save, not as an aborted case at run time") and every earlier gate contributed its `isBadRequest()` leg; the G-11 commit's gate skipped the ritual, so a regression could silently re-open C-49P1's hole with CI green. **Fixed (V-C49P2)**: six legs join the method — non-Task entity, `count: 0`, `timeoutMs: 2.5`, `count: true`, `timeoutMs: 120000` (each `isBadRequest()`), and the reference-shaped `${Query[0].count}` with an absent `timeoutMs` still legal (`isOk()`). **Bite-proven**: with `DefinitionService`'s fix stashed and the new legs in, the method fails (`Tests run: 19, Failures: 1` — the fraction leg's stored-green 400-expectation); restored, 19/19.

### C-49P3 — the Status paragraph's "latest counts" still read Java (730) after the feat commit grew `TestRunnerJourneyTests` from three `@Test`s to four (the poll + timeout legs): the count the forty-eighth pass verified as "exactly the README's count claim" stopped being true before that pass's commit even landed its successor

**Fixed (V-C49P3)**: recounted from this pass's own run — see Verification — and re-pointed to 731.

### Verification (this pass)

- Full `./mvnw verify` BUILD SUCCESS on the podman socket across the reactor on the final tree (all three fixes in, the regenerated artifacts committed): **731 Java tests** green, counted from this run's own surefire reports across exactly the twenty test-bearing modules (6 stale reports from earlier runs excluded by mtime) — 730 + `TestRunnerJourneyTests`' fourth leg — with all five e2e cycle tests green end to end (ErpSuiteCorpus 1, O2C 1, P2P 1, R2R 2 = 5, the closeChecklist suite riding the corpus live) and zero orphaned `SNAPSHOT.jar` processes after the runs.
- The dogfood gates green on the final tree: `BuildrightAppArtifactTests` 13/13, `BuildrightWorkflowRegisterTests` 2/2, `ErpAppArtifactTests` 12/12, `DefinitionLifecycleTests` 19/19 (the six new awaitTasks legs included), `TestRunnerJourneyTests` 4/4.
- `pnpm -r check` + `pnpm -r test` green: **302 frontend tests** (shared 184 — 118 static + the 66-case expr/v1 conformance corpus the shared runner drives dynamically, builder-ui 74, runtime-ui 44 — exactly the README's count; no frontend surface touched).
- `deploy/helm/check-charts.sh` CLEAN (env parity across URL/HOST/KAFKA/TEMPO, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` clean (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` CLEAN.
- Regeneration byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev` — the artifacts now record the HEAD rev `1fba240` the committed copies had not yet recorded); counts reconcile (10/225/5,214 = 5,449; 1/188/539 = 728; must-have 1/128/300 = 429; byWave `{'1': 85, '2': 209, '3': 147, '4': 315, 'None': 4,529, 'edge': 113, 'platform': 51}`); the Waves table's enumerating rows re-confirmed set-equal against `coverage-map.json` (wave 4's 22 prefixes; edge's `ECOM/LOG/POS/WMS`).
- Whole-repo sweeps: all relative markdown links resolve (84 checked); zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; the only remaining "730"s are this file's historical verification records; the harness `awaitTasks` documentation (PHASE-4 §12, `TestSuiteDefinition`'s OPS javadoc, `TestRunner`'s method javadoc, the two gap-log G-11/G-17 dispositions, the ledger's dated amendment) agrees with the implementation on every number (200 ms poll, default 20 s, 60000 cap, `AWAIT_TIMEOUT`).

### Recorded open after this pass

Empty. One considered-and-dismissed: the save gate does not shape-check `awaitTasks`' `status` slot the way `queryRecord`'s filter is shape-checked, because the run time itself does not validate it (`.toString()` into the inbox query, where the Workflow Service's own contract answers) — there is no run-time contract to mirror, and inventing one at save time would create the asymmetry this pass removes, not close one.

---

## Fiftieth Pass — 2026-09-10 (the harvest's surfaces met neither edge validation nor their own twins: the G-17 commit's start surface NPE'd a missing tenantId into a 500 where its sibling pins the 400 convention, its javadoc's "no gateway route" contradicted the gateway config and the spec's own landscape table, its new op never reached the TS twin's SUITE_OPS — the builder could not author a step the backend's own save gate accepts — and no gate pinned the two vocabularies together)

### C-50P1 — `InternalProcessController.start` validated `app` and `process` but not `tenantId`: an absent tenant sailed past the guard and NPE'd inside `UUID.fromString(null)` — a 500 INTERNAL with a logged stack trace, where the surface's sibling `InternalSlaController.scan` pins the exact check (`"sla scan requires tenantId"` → 400) and the controller's own app/process check establishes the convention the surface itself claims

The scheduler's `RestProcessTarget` always sends a tenantId, so the hole is reachable only by a direct service-token caller — but the surface's contract is edge validation (the same convention PHASE-4 §12's gates pin: authoring errors surface at the door), and `ProblemAdvice` maps `IllegalArgumentException` to 400 while an NPE falls to the catch-all 500, so a caller omitting one field got the one response shape the surface never intends, plus an ERROR-level log line per attempt. **Fixed (V-C50P1)**: the guard now mirrors the sibling's — null/blank `tenantId` throws `VALIDATION_FAILED` ("process start requires tenantId") before any parse. **Bite-proven**: with the fix stashed and the new leg in, `BpmnProcessTests.internalStartSurface` fails (`Status expected:<400> but was:<500>`, the missing-tenantId leg); restored, 15/15.

### C-50P2 — the same leg shipped without the rejection pins every other internal surface carries, and the approval surface's javadoc claimed a topology property the gateway config contradicts: "no gateway route, no user traffic" — while the gateway's `Path=/api/v1/workflow/**` route reaches the entire internal prefix (PHASE-4 §4's own landscape table says so) and the `ServiceClientGate` is the actual control refusing every user token with 403

The pass-30 class in reverse: the wire is checked (the 403 legs are pinned in `TaskApiTests` and `BpmnProcessTests`) but the prose told a different story than the config — a security-adjacent inaccuracy this project's own history keeps paying down. **Fixed (V-C50P2)**: the approval controller's javadoc now states the true topology (the route does reach the prefix; the gate is what keeps user traffic off it, the pinned legs named) and the process controller's matches; plus the two rejection legs the start surface shipped without — service-client POST without `tenantId` → 400, with a malformed `recordId` → 400 (the `ProblemAdvice` `IllegalArgumentException` mapping this surface relies on, now pinned on this surface) — riding `internalStartSurface` like the sla/scan surface's authoring-error legs ride its journey.

### C-50P3 — the G-11 harvest grew the vocabulary on one side only: `awaitTasks` joined `TestSuiteDefinition.Step.OPS` (the save gate and the runner's dispatch switch) but never the TS twin's `SUITE_OPS` — the builder's suite editor renders its op dropdown from the twin, so the authoring UI could not author a step the backend's own save gate accepts and the corpus's newest suite rides

The mirror image of the thirty-fifth pass (page-bind rules living only in the TS twin): this time the server grew and the twin lagged, and no gate watched the pair — the expression DSL's parity has the shared conformance corpus, the suite-op vocabulary had nothing. **Fixed (V-C50P3)**: `awaitTasks` joins `frontend/shared/src/metadata.ts`'s `SUITE_OPS` (twin javadoc naming the lockstep), and a new `SuiteOpsLockstepTest` (metadata-model) reads the twin's source and pins the two sets equal — failing loudly on whichever side moves first. **Bite-proven**: with the twin's `awaitTasks` line removed, the lockstep fails (`Expecting actual: … ["awaitTasks"]`); restored, green.

### C-50P4 — the e2e ledger's corpus paragraph still ended its amendment trail at "the corpus is eight suites" (2026-09-06): the 2026-09-09 ninth suite (`closeChecklist`) is recorded in the register's amendment further down the file, but the corpus section's own amendment chain — which tracked every prior growth — missed the growth it describes

**Fixed (V-C50P4)**: a dated amendment appends the ninth suite's landing (the G-11/G-17 harvests, all nine per boot, the register gate's corpus list riding), completing the trail the section keeps.

### Verification (this pass)

- The reactor on the final tree: **BUILD SUCCESS** through all twenty test-bearing modules and the e2e module's other four JVMs; the corpus JVM's Keycloak container timed out on port readiness (an environment boot flake, not a code leg — P2P 1, O2C 1, R2R 2 ran green inside the failed reactor) and **re-ran green** in isolation with the stack booted fresh (ErpSuiteCorpus 1, the `closeChecklist` suite riding the corpus live against this pass's javadoc-changed workflow jar). Recounted from this session's surefire reports on the final tree (mtime-filtered, 125 report files): **732 Java tests, 0 failures** across exactly the twenty test-bearing modules — 731 + the new `SuiteOpsLockstepTest`; the two `internalStartSurface` legs ride inside their method, so the surface's count is unchanged. Zero orphaned `SNAPSHOT.jar` processes after the runs.
- `pnpm -r check` + `pnpm -r test` green: **302 frontend tests** (shared 184, builder 74, runtime 44 — unchanged; `SUITE_OPS` has no TS-side count, the lockstep gate is the Java-side pin).
- The four deploy gates CLEAN: `check-charts.sh` (lint + render 12 charts, env parity, DNS 18/18, 31 pods on named token-less ServiceAccounts, 13 charts default-deny, 30 disruption budgets); `check-observability.sh` (11 rows × 3 panels, no collisions); `ops-selftest.sh` CLEAN; `check-build-tree.sh` CLEAN.
- Both registers regenerate byte-stable apart from the designed stamps (`generatedAt`/`novaforgeRev`/`erpplansRev` — now recording the HEAD rev `5a01248`; the erpplans checkout's HEAD hash moved `dd49ecf0` → `8f308087` with byte-identical content, which the unchanged register bodies prove); counts reconcile (10/225/5,214 = 5,449; 1/188/539 = 728; tiers 1,396/3,295/758).
- Whole-repo sweeps: all relative markdown links resolve (84 checked); zero TODO/FIXME markers in shipped source; deploy scripts `bash -n` clean; the README Status count re-pointed 731 → 732 from this run's own reports.

### Recorded open after this pass

Two considered-and-dismissed. (1) The scheduled deployer's `sync()` and the harness-driven `syncOnce()` can run concurrently: both compute the same desired set from the same source, `beginSync` upserts idempotently, and the `deploymentId == null` guard (documented as crash recovery) self-heals the only visible effect — a transient duplicate engine deployment of identical content, absorbed by `markDeployed`'s history append and corrected by the next pass; a lock would trade a benign, self-healing race for serialized sync latency on the suite path. (2) `awaitTasks`' `status` slot still carries no save-gate shape check — the pass-49 dismissal stands (the run time does not validate it; inventing a save-time contract would create the asymmetry pass 49 removed).
