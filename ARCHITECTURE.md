...
### 2.1 API Gateway
- Spring Cloud Gateway; routes `/api/v1/metadata/**`, `/api/v1/runtime/**`, `/api/v1/workflow/**`, `/api/v1/notifications/**`, `/api/v1/reports/**`, `/api/v1/integrations/**`, `/api/v1/files/**`, `/api/v1/audit/**`, `/api/v1/admin/**`, `/api/v1/webhooks/inbound/**` (plus `GET /api/v1/scheduler/jobs` — the Scheduler's sole read-only gateway path, PHASE-4 §2), all under `/api/v1/` (versioning rule: §6)
- JWT validation (Keycloak JWKS), tenant header derivation (`X-Tenant-Id` from token claim), rate limiting via Redis — the limiter itself activates with Phase 6's first public route (PHASE-0 §6.1's deferral; PHASE-6 §6), not at Phase 0. The default JWT requirement has exactly one API-route exception — the anonymous inbound-webhook prefix that arrives with Phase 6 (PHASE-6 spec §2/§6), rate-limited from its first day. **Limiter-outage posture (pinned 2026-09-03): fail closed** — a limiter backend outage renders 503 on this one anonymous route rather than waving it through (an unauthenticated route is exactly where throttling matters most; senders replay per the delivery log; the HMAC verification behind the route never depended on the limiter). The prior fail-open behavior remains available as an explicit deployment choice (`novaforge.webhook.rate-limit-fail-open:true`) — a decision, not a default. Authenticated routes carry no per-user rate limits in v1 — a demand-driven backlog item, stated here so its absence is a decision, not an omission.

### 2.2 Identity Service
- Keycloak realm strategy — decided (PHASE-0 §12 Q1, resolved; recorded in [ADR-002](./docs/adr/ADR-002-authn-keycloak-authz-platform.md)): single realm `novaforge` + tenant claim; realm-per-tenant isolates better but is revisited only at true tenant-isolation requirements.
- Roles: platform roles (`admin`, `builder`, `user`) + app-defined roles. Two mechanisms were considered — syncing app-defined roles into Keycloak via its Admin API, or an Authz module inside Data Runtime backed by the platform DB; the decision below (ADR-002, accepted) picks the latter: authorization resolves in the platform, Keycloak stays authentication-only.
- Decision (ADR-002): Keycloak handles *authentication only*; Data Runtime handles *authorization* (roles stored in platform DB) — simpler than syncing dynamic roles into Keycloak.

### 2.3 Metadata Service (design-time)
- Owns: `AppDefinition`, `EntityDefinition`, `FieldDefinition`, `RelationshipDefinition`, `PageDefinition` (pages pin catalog component versions; the component catalog itself — registry entries carrying props schemas, lifecycle, and versions — is versioned metadata on the same branch, v1 shipping the platform's built-ins as its entries: ADR-009 #1, PHASE-2 §6), `RuleDefinition` (event-hook and scheduled-job rules — the Business Rules branch of PLAN.md §2; the cron *registry* stays with the Scheduler, §2.8), `WorkflowDefinition`, `StateMachineDefinition`, `SLADefinition`, `SharingRuleDefinition` (the PermissionSet branch — all four land in Phase 4, PHASE-4 spec §2; WorkflowDefinition has been listed since v0, the rest join it then), `ReportDefinition`, `DashboardDefinition`, `PermissionSet` (app role definitions + object/field/record-rule security — the Permissions branch of PLAN.md §2; user→role assignments are tenant data, not promoted metadata — PHASE-2 spec §9), `TestSuiteDefinition` (builder test suites — [ADR-010](./docs/adr/ADR-010-builder-test-harness.md)), plus connector, webhook, import, credential-reference, and API-client definitions (the Integrations branch of PLAN.md §2; API clients themselves stay deferred with demand — PHASE-6 spec §1; `CredentialDefinition` carries the reference only — the secret material stays in the secrets store, never in metadata: PHASE-6 spec §2/§9), sandboxed-script artifacts (versioned with the same review/promotion path as definitions — ADR-008 #4), and app-scoped settings definitions (sequences, currencies, localization, shared enums — the Settings branch of PLAN.md §2; sequence *execution* stays with the Data Runtime per PLAN.md §3).
- Validates definitions on save (schema validation + referential integrity, e.g., formula references exist).
- Hosts the **test runner** per [ADR-010](./docs/adr/ADR-010-builder-test-harness.md): executes builder test suites against a scratch tenant pinned to a *candidate* version (ADR-010 #3 — the immutable snapshot a publish creates from the dev draft workspace; the runtime never serves mutable drafts) — steps run as synthetic actors through the Data Runtime's generic APIs and, for the Phase 4 vocabulary, the Workflow inbox API (no test mode in the write path), and run artifacts are bound to the exact definition version; green runs gate change-set promotion (PLAN.md P8).
- On publish: bumps version, writes to `metadata_versions`, emits `metadata.published` on the Kafka spine (cache invalidation and the §4 storage materializer both react to this one event; the Phase 1 Redis pub/sub interim was retired with the Phase 3 rebind — PHASE-1-METADATA-CORE.md §4, PHASE-3-BUSINESS-LOGIC.md §4).
...
- Large exports run async (on-demand Integration-Service jobs — PHASE-6 spec §7; the Scheduler's target set has no export kind) streaming to file service (File Service lands in Phase 6 — direct downloads capped at 10k rows until then: PHASE-5 spec §6).
...
| 005 | Monorepo, Maven, Java 21 (Boot/Cloud versions: ADR-007) | Accepted — [ADR-005](./docs/adr/ADR-005-monorepo-maven-java21.md) |
| 006 | Multi-tenancy: shared schema + RLS | Accepted — [ADR-006](./docs/adr/ADR-006-shared-schema-rls.md) |
