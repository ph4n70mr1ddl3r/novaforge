...
**The workflow register (2026-09-08) — the dogfood bound to all of erpplans' operational workflows (PHASE-7 §12.3a).** Coverage discipline rose from requirements to the workflows themselves: `apps/buildright/scripts/generate-workflow-coverage.py` reads every `## W` header across the 569 PA files (5,426 canonical workflows), joins each with its criticality tier from `workflow-criticality-classification.md` (all 5,449 register rows — the 23 `###` parent/summary sub-workflows ride marked — reconciling the canon split 1,396/3,295/758 exactly) and with its requirement links from `requirement-workflow-matrix.md`, derives each workflow's state from the committed requirement coverage, and merges the hand-maintained pins (`workflow-map.json`). Every row lands in exactly one of three states: **pinned** (the workflow's core cycle executes green in the e2e corpus — the entry names the suite files; the register seeds W2/W2A/W3/W7 via the P2P suites and W8/W9A/W308 via the erp corpus, with W9 partial — its checklist core is gap-logged G-10/G-11), **partial** (a primary-linked requirement is claimed; the requirement's evidence and gaps travel with the derived note; 228 rows), or **uncovered** (visible, wave-attributed; 5,214 rows). The honesty gate `BuildrightWorkflowRegisterTests` pins the canons (5,426 / 5,449 / 1,396-3,295-758 — upstream growth fails loudly), every pin naming a suite that both exists and rides the live thirteen-suite e2e corpus, and every partial naming an existing app — bite-proven: a pin naming a corpus-external suite fails the corpus-membership leg, a pin naming a nonexistent suite fails the existence leg, the restored register regenerates green. Specs updated in place: §12.3a added, §12.4's wave-1 line names its register pins, PLAN.md's traceability bullet and both READMEs re-pointed. *(Amended 2026-09-08, forty-fifth pass: the body's "partial — 227 rows" miscounted the register at authoring time — the committed artifact, the gate's pins, and this landing's own commit message all say 228; re-pointed.)* *(Amended 2026-09-08, forty-fourth pass: the "uncovered — wave-attributed" phrasing above overclaims — 4,529 of the 5,214 uncovered rows ride `null` waves because the upstream requirement-workflow matrix covers the core value streams incrementally; the honest wording — wave-attributed where the matrix links them — now sits in §12.3a, both READMEs, PLAN, and the generator, and the gate gained the map-sync legs noted in CODE_REVIEW.md.)*

Verified: `BuildrightWorkflowRegisterTests` + `BuildrightAppArtifactTests` green (metadata-service, 14 tests); generator idempotent against the canons; no e2e-module or product Java touched (the register is test-side plus generated artifacts). *(Amended 2026-09-09, the register's first growth increment: the pin count moves 7 → 9 — W638 (period-end journal-entry review & approval, wave 2) and W889 (AR aging analysis & collection prioritization, wave 3) pin via the erp suites that already execute their core cycles, partial re-counts 228 → 226 — and W2's unsuite-pinned close leg is now pinned too: a new `po-close-edge-the-machine-terminal-contract` case rides `p2pReceivingBillingEdges` live (the APPROVED→CLOSED edge, unlisted jumps and post-CLOSED transitions refused `STATE_TRANSITION`), so the W2 pin names both P2P suites. The next increment needs the G-11 harvest — a harness await/poll step op — before W9's checklist core can be suite-pinned.)* *(Amended 2026-09-09, the register's second growth increment — the G-11 harvest landed and W9's checklist core is pinned: the harness grows the `awaitTasks` poll op (bounded 200 ms retries over the inbox query, rows and `{count, ids}` landing exactly where `queryRecord` puts them, a timed-out poll answering `AWAIT_TIMEOUT` — pin-able with `expect: error(AWAIT_TIMEOUT)`), and authoring the suite leg surfaced a second gap live, **G-17**: the deployer's scheduled sync (30 s default) let a triggering write beat its own event-started workflows into the registry and `ProcessStarts` evaluates deployed subscriptions only, so the skipped start never retried — closed by the service-client-gated internal `POST /api/v1/workflow/internal/processes/sync` surface, which the harness now drives right after publishing the candidate, making event-start legs deterministic for every suite. The new `closeChecklist` suite rides both harvests live: the CLOSING event-start forks the three reconciliation tasks (awaited per role), the parallel join holds the controller's `confirmClose` back until all three resolve — pinned by the empty controller inbox mid-checklist (`${Query[3].count} == 0`) — the confirmed close lands CLOSING→CLOSED; W9 pins `closeChecklist.json` + `controls.json`, pinned 9 → 10, partial 226 → 225, and the e2e corpus grows to nine suites (the register gate's corpus list with it). Verified: `TestRunnerJourneyTests` (4, the poll + timeout legs included), `BpmnProcessTests` (15, the sync surface's user-token 403 leg included), `ErpAppArtifactTests` + `BuildrightAppArtifactTests` + `BuildrightWorkflowRegisterTests` green, and the full e2e module (5 tests) green against the booted stack with the new suite executing live.)*
