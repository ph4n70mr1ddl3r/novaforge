...
- Traceability is the definition of done: the committed coverage matrix maps every one of the 728 requirements to app metadata or a visible gap, and the committed workflow register (§12.3a) puts all 5,449 erpplans workflow rows in exactly one state — pinned by the e2e corpus, partial via a claimed requirement, or uncovered — wave-attributed where the upstream requirement-workflow matrix links them, honestly null where it does not yet; `BuildrightAppArtifactTests` gates the artifact, the suites, and the matrix's honesty, and `BuildrightWorkflowRegisterTests` gates the register
