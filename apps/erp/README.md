...
| `suites/closeChecklist.json` | The §4/§9-item-3 close checklist, end to end (the G-11/G-17 harvests): the CLOSING event-start forks the three reconciliation tasks (`awaitTasks` per role — the bounded poll over the inbox), the parallel join holds the controller's `confirmClose` back until all three resolve (pinned by the empty controller inbox mid-checklist), the confirmed close lands CLOSING→CLOSED |
...
- **Periods** — `AccountingPeriod` `OPEN → CLOSING → CLOSED` with the audited
  ...
  only with all tasks resolved, and the `closeChecklist` suite pins the whole ride
  end to end (the G-11/G-17 harvests: the `awaitTasks` poll op + the harness's
  post-publish deployment sync — the reopen-approval leg alone stays gap-logged:
  G-10).
